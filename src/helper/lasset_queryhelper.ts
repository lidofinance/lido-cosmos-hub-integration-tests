...
export const makeRestStoreQuery = async (
  ...
  console.log(
    'query',
    `/cosmwasm/wasm/v1/contract/${contract_address}/smart/${Buffer.from(
      JSON.stringify(msg),
    ).toString('base64')}`,
  );
...
class TokenQuerier {
  ...
  constructor(token_address: string, wrapper: CosmosWrapper) {
    this.token_address = token_address;
    this.wrapper = wrapper;
  }

  async query(msg: object): Promise<any> {
    return makeRestStoreQuery(this.token_address, msg, this.wrapper.lcdClient);
  }
  ...
  public async token_info(): Promise<TokenInfoResponse> {
    return this.query({
      token_info: {},
    }).then((r) => r.data as TokenInfoResponse);
  }
...
export default class LidoAssetQueryHelper {
  ...
  wrapper: CosmosWrapper;
  constructor(wrapper: CosmosWrapper, lasset: LidoAsset) {
    this.wrapper = wrapper;
    this.lasset = lasset;
  ...
  public async balance_statom(
    address: string | DirectSecp256k1HdWallet,
  ): Promise<number> {
    if (typeof address !== 'string') {
      address = (await address.getAccounts())[0].address;
    }
  ...
  public async get_validators_for_delegation(): Promise<Array<QueryValidator>> {
    ...
    }).then((r) => r.data as Array<QueryValidator>);
  }
  ...
  public async all_history(
    ...
    }).then((r) => r.data as AllHistoryResponse);
  }
  ...
  public async get_withdraweble_unbonded(
    ...
    const latestBlock = await this.wrapper.tmClient.block();
    return this.lassethubquery({
      withdrawable_unbonded: {
        address: address,
        block_time: Math.trunc(latestBlock.block.header.time.getTime() / 1000),
      },
