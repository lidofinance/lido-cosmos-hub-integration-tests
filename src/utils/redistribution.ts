...
const incoming_redelegations_inprogress = async (
  ...
  let [redelegations, pagination] = await lcd.staking.redelegations(
    hubContract,
  );
  let validators_address = validators.map((v) => {
    return v.validator;
  });
...
export const redistribute = async (
  ...
  let inprogress = await incoming_redelegations_inprogress(
    lcd,
    hubContract,
    validators,
  );
  ...
  for (let i = 0; i < redelegations.length; i++) {
    let r = redelegations[i];
    await redelegate_proxy_multisig(
      lcd,
      hubContract,
      multisigPubkey,
      keys,
      r.srcVal,
      [[r.dstVal, new Coin(atomDenom, r.amount)]],
    );
  }
};
...
export const redelegate_proxy_multisig = async (
  ...
  const msg = new MsgExecuteContract(multisigAddr, hubcontract, {
    redelegate_proxy: {
