...
export const vals = [
  {
    name: "terradnode0",
    address: "wasmvaloper188p7d0w6948y8p4cg5p3m6zx8lzzg8r0vt47ms",
  },
  {
    name: "terradnode1",
    address: "wasmvaloper180darp2tj7ns48r0s3l3u8a2ygxjyycsjmyhzz",
  },
  {
    name: "terradnode2",
    address: "wasmvaloper1utdag7jnhp9zy667z78dt8hnnud2mu7vax5rsn",
  },
  {
    name: "terradnode3",
    address: "wasmvaloper1yg247q4kecqnktp2rte030yy43gpj0c9nm5nug",
  },
];
...
/* 
Validators:
wasmvaloper188p7d0w6948y8p4cg5p3m6zx8lzzg8r0vt47ms - node0 192.168.10.2
...
wasmvaloper180darp2tj7ns48r0s3l3u8a2ygxjyycsjmyhzz - node1 192.168.10.3
...
wasmvaloper1utdag7jnhp9zy667z78dt8hnnud2mu7vax5rsn - node4 192.168.10.4
...
wasmvaloper1yg247q4kecqnktp2rte030yy43gpj0c9nm5nug - node3 192.168.10.5
...
const { DOCKER_NETWORK = "testkit_localnet" } = process.env;
...
export const disconnectValidator = async (name: string): Promise<any> => {
  const { stdout, stderr } = await exec(
    `docker network disconnect ${DOCKER_NETWORK} ${name}`
  );
  return stdout;
};
...
export class TestStateLocalTestNet {
  ...
  constructor() {
    ...
    this.wallets = {
      valAWallet: this.lcdClient.wallet(
        new MnemonicKey({ mnemonic: ValidatorsKeys[0] })
      ),
      valBWallet: this.lcdClient.wallet(
        new MnemonicKey({ mnemonic: ValidatorsKeys[1] })
      ),
      valCWallet: this.lcdClient.wallet(
        new MnemonicKey({ mnemonic: ValidatorsKeys[2] })
      ),
      valDWallet: this.lcdClient.wallet(
        new MnemonicKey({ mnemonic: ValidatorsKeys[3] })
      ),

      ownerWallet: this.lcdClient.wallet(
        new MnemonicKey({ mnemonic: accKeys[0] })
      ),
      ...
      lido_fee: this.lcdClient.wallet(
        new MnemonicKey({ mnemonic: accKeys[4] })
      ),
      gasStation: this.lcdClient.wallet(
        new MnemonicKey({ mnemonic: accKeys[5] })
      ),
    };
    ...
    this.multisigPublikKey = new LegacyAminoMultisigPublicKey(
      2,
      this.multisigKeys.map((mk) => {
        return mk.publicKey as SimplePublicKey;
      })
    );
  }
  ...
  async init() {
    ...
    console.log("going to store contracts");
    console.log(Object.values(this.wallets).map((w) => w.key.accAddress));
...
export const get_expected_sum_from_requests = async (
  querier: LidoAssetQueryHelper,
  reqs: UnbondRequestsResponse
): Promise<number> => {
  return reqs.requests.reduce(async (acc, [batchid, amount_statom_tokens]) => {
    ...
    if (h.history.length == 0) {
      ...
      return (
        Number(h.history[0].statom_withdraw_rate) *
          Number(amount_statom_tokens) +
        acc_sum
      );
    }
  }, Promise.resolve(0));
